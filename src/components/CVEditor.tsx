...
export default function CVEditor() {
  const { currentCV, savedCVs, updateCV } = useApplicoStore();
  ...
  useEffect(() => {
    if (savedCVs.length > 0 && selectedIndex < savedCVs.length) {
      setCv({ ...savedCVs[selectedIndex], importedFrom: savedCVs[selectedIndex].importedFrom || {}, aiSuggestions: savedCVs[selectedIndex].aiSuggestions || [] });
    } else {
      setCv(currentCV);
    }
  }, [selectedIndex, savedCVs, currentCV]);
  ...
  const handleSave = () => {
    if (cv && savedCVs.length > 0) {
      updateCV(selectedIndex, cv);
      setSaved(true);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {savedCVs.length > 1 && (
            <select value={selectedIndex} onChange={(e) => setSelectedIndex(parseInt(e.target.value))} className="px-3 py-2 border border-navy-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy-500 text-navy-900">
              {savedCVs.map((s, i) => <option key={i} value={i}>{s.fullName || `CV ${i + 1}`}</option>)}
            </select>
          )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-navy-100 p-6 space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={addSkill} className="text-xs font-medium text-navy-600 hover:text-navy-800">+ Add</button>
            </div>
            <div className="flex gap-2">
              <input type="text" value={skillInput} onChange={(e) => setSkillInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addSkill(); } }} placeholder="Add skill" className="flex-1 px-3 py-2 border border-navy-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy-500 text-navy-900 placeholder-navy-300" />
            </div>
    ...
    <div className="space-y-4">
      ...
      {cv.skills.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">{cv.skills.map((s, i) => <span key={i} className="px-2 py-0.5 bg-navy-100 text-navy-700 text-xs rounded">{s}</span>)}</div>
        </div>
      )}
      {cv.experience.length > 0 && (
        <div>
          <h4 className="text-xs font-bold text-navy-800 uppercase tracking-wide mb-2">Experience</h4>
          <div className="space-y-3">{cv.experience.map((exp, i) => (
            <div key={i}><p className="text-sm font-semibold text-navy-900">{exp.role}</p><p className="text-xs text-navy-600">{exp.company} | {exp.startDate} - {exp.endDate}</p>{exp.description && <p className="text-xs text-navy-600 mt-1">{exp.description}</p>}</div>
          ))}</div>
        </div>
      )}
      {cv.education.length > 0 && (
        <div>
          ...
          <div className="space-y-2">{cv.education.map((edu, i) => (
            <div key={i}><p className="text-sm font-semibold text-navy-900">{edu.degree} {edu.field && `in ${edu.field}`}</p><p className="text-xs text-navy-600">{edu.institution} | {edu.startDate} - {edu.endDate}</p></div>
          ))}</div>
        </div>
      )}
    </div>
...
function formatATS(cv: CVData): string {
  ...
  if (cv.experience.length > 0) {
    ...
    cv.experience.forEach((exp) => { lines.push(`${exp.role} - ${exp.company}`); lines.push(`${exp.startDate} - ${exp.endDate}`); if (exp.description) lines.push(exp.description); lines.push(""); });
  }
  if (cv.education.length > 0) {
    lines.push("EDUCATION");
    cv.education.forEach((edu) => { lines.push(`${edu.degree} in ${edu.field} - ${edu.institution}`); lines.push(`${edu.startDate} - ${edu.endDate}`); lines.push(""); });
  }
  return lines.join("\n");
}
